"""Application (app-only) permission keşfi testleri."""
import collectors
import report
import scoring
...
def test_app_permission_resolution_and_fallback():
    graph = FakeGraph(
        assignments={"app1": [
            ...
            {"appRoleId": ZERO, "resourceId": "graph"},  # rol yok → atlanır
        ]},
        resources={
            "graph": {"displayName": "Microsoft Graph",
                      "appRoles": [{"id": "r-known", "value": "Sites.ReadWrite.All"}]},
            "custom": {"displayName": "Contoso HR API", "appRoles": []},  # çözülemez
        })
    apps = [{"sp_id": "app1"}]
    ...
    assert len(perms) == 2  # zero-guid atlandı
    # GUID → okunabilir ad çözüldü
    assert {"resource": "Microsoft Graph", "permission": "Sites.ReadWrite.All",
            "permission_id": "r-known"} in perms
    # Çözülemeyen custom API permission GUID'i KAYBOLMADI
...
def test_app_only_app_has_no_delegated():
    ...
    collectors.enrich_with_oauth_grants(FakeGraph(), apps)   # delegated yok
    collectors.enrich_with_app_role_assignments(graph, apps)
    assert apps[0]["has_app_only_access"] is True
...
def test_delegated_permissions_structured():
    ...
    assert "files.read.all" in apps[0]["scopes"]  # eski çıktı korundu
...
def test_scoring_counts_app_only_permissions():
    ...
    assert app["risk_score"] > 0                       # eskiden 0'dı — kör nokta kapandı
...
def test_report_renders_app_only_cards_and_filter():
    apps = [{"display_name": "AppOnlyBot", "vendor": "X", "first_party_microsoft": False,
             ...
             "user_count": 0, "risk_score": 40, "risk_level": "Orta", "reasons": ["r"],
    ...
    assert "App-only izinler (kullanıcısız)" in doc      # bulgu bloğu
